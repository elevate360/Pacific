 ...
 */
 ...
 class el_portfolios extends el_content_type{
	...
	public static function el_display_portfolio_tiles($optional_args = array()){
		...
		//defaul output will be a tile based listing
		if(!isset($optional_args['display_type'])){
			$optional_args['display_type'] = 'tile';
		}
		...
		if($posts){
			...
			if($optional_args['display_type'] == 'tile'){
				$html .= '<article class="portfolio-tiles animation-container equal-height-items el-row  small-margin-top-bottom-medium">';
			}else if($optional_args['display_type'] == 'card'){
				$html .= '<article class="portfolio-cards animation-container equal-height-items el-row  small-margin-top-bottom-medium">';
			}

				
				//output a header section before listing 
				if(isset($optional_args['title']) || isset($optional_args['subtitle'])){
					$html .= '<div class="el-col-small-12 small-align-center">';
					//title
					if(isset($optional_args['title']) && !empty($optional_args['title'])){
						$html .= '<h1 class="big fat">' . $optional_args['title'] . '</h1>';
					}
					//subtitle
					if(isset($optional_args['subtitle']) && !empty($optional_args['subtitle'])){
						$html .= '<h2 class="big">' . $optional_args['subtitle'] . '</h2>';
					}
					$html .= '</div>';
				}
							...
							$html .= '<div class="content-wrap" style="' . $style . '">';
								...
								if(!empty($post_portfolio_logo)){
									...
									if($post_attachment->post_mime_type == 'image/svg+xml'){
										...
										$image_url = wp_get_attachment_image_src($post_portfolio_logo, 'large', false)[0];
									}
